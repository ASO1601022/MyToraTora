...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (id){
            case R.id.gu:
                ...
                break;
            case R.id.choki:
                myHandImageView.setImageResource(R.drawable.granma);
                myHand = JANKEN_CHOKI;
                break;
            case R.id.pa:
                myHandImageView.setImageResource(R.drawable.katokiyomasa);
                myHand = JANKEN_PA;
                break;
            default:
                myHand = JANKEN_GU;
        ...
        //コンピュータの手を決める
        int comHand = getHand();
        ImageView comHandImageView = (ImageView)findViewById(R.id.com_hand);
        switch (comHand){
            case JANKEN_GU:
                comHandImageView.setImageResource(R.drawable.tiger);
                break;
            case JANKEN_CHOKI:
                comHandImageView.setImageResource(R.drawable.granma);
                break;
            case JANKEN_PA:
                comHandImageView.setImageResource(R.drawable.katokiyomasa);
        }

        //勝敗を判定する
        TextView resultLabel = (TextView)findViewById(R.id.judge);
        int gameResult = (comHand - myHand + 3) % 3;
        switch (gameResult){
            case 0:
                //あいこの場合
                resultLabel.setText(R.string.draw);
                break;
            case 1:
                //勝った場合
                resultLabel.setText(R.string.win);
                break;
            case 2:
                //負けた場合
                resultLabel.setText(R.string.lose);
                break;
        }
        //ジャンケンの結果を保存する
    ...
    private int getHand(){
        ...
        if(gameCount == 1){
            if(gameResult == 2){
                //前回の勝負が１回目でコンピュータが勝った場合、コンピュータは次に出す手を変える
                while(lastComHand == hand){
                    hand = (int)(Math.random() * 3);
                }
            }else if(gameResult == 1){
                //前回の勝負が１回目でコンピュータが負けた場合、相手の出した手に勝つ手を出す
